...
이 Part는 context를 프롬프트 텍스트가 아니라 운영 자원으로 읽고, turn loop와 recovery를 상태 기계로 해석합니다. 먼저 context class, compaction, handoff, stop hook 같은 일반 원리를 잡고, 이어서 Claude Code의 query pipeline과 QueryEngine이 그 원리를 어떻게 구현하는지 봅니다.

## 이 Part의 핵심 질문

- context는 어떤 lifetime과 owner를 가져야 하는가
- compaction, memory, handoff artifact는 왜 따로 다뤄야 하는가
