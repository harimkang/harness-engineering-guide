...
이 장의 reader-facing 외부 검증 축은 [../00-front-matter/03-references.md](../00-front-matter/03-references.md)의 `S6`, `S10`, `S14`, `S24`를 따른다. `S13`은 startup contract의 headless/owner variant 비교에만 보조적으로 걸친다.
...
제품 사실: trust가 확인되면 `src/interactiveHelpers.tsx` 안에서 session trust 상태, GrowthBook, system context 같은 trust-immediate initialization이 시작되고, `mcp.json`과 external include 같은 trust-adjacent approval도 같은 startup 구간 안에서 이어진다. 이후 repo-path mapping, full environment 적용, telemetry 초기화가 계속되며, `src/main.tsx`로 복귀한 뒤에도 LSP 초기화와 일부 entitlement 해석처럼 trust 이후에만 안전한 작업이 추가로 이어진다.  
해석: post-trust initialization은 하나의 block이 아니라 단계적으로 열리는 초기화 집합이다. 다만 이 순서는 개념적 층위 구분으로 읽는 편이 정확하며, 모든 세부 동작이 하나의 엄격한 선형 파이프라인으로 고정된다고 보는 것은 과하다.

이 점이 중요하다. approval dialog 몇 개가 남아 있다고 해서 post-trust initialization이 아직 시작되지 않은 것이 아니다. Claude Code의 실제 startup은 `trust 확인`, `trust-immediate initialization`, `trust-adjacent approval`, `later post-trust initialization`, `mode/capability gate`라는 개념적 층위가 부분적으로 겹치며 열린다고 보는 편이 정확하다.
