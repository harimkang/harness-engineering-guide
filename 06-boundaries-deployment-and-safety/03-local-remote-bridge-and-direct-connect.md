...
이 장의 reader-facing 외부 검증 축은 [../00-front-matter/03-references.md](../00-front-matter/03-references.md)의 Part 6 cluster를 따른다. deployment family, auth handshake, data path, version-negotiation framing에는 `S5`, `S9`, `S15`, `S20`, `S30`, `S31`을 우선 사용한다. `P1`은 boundary artifact 비교의 보조 프레임으로만 사용한다.
...
최신 배포 문맥에서는 여기서 transport와 authz 경계도 같이 적는 편이 좋다. streamable HTTP, WebSocket, tunnel relay, OAuth/token-bearing session contract는 모두 "원격"이라는 한 단어보다 더 많은 의미를 갖는다. 특히 direct-connect와 attach family는 누가 session contract를 발급하는지와 누가 authorization response를 되돌리는지가 다르다.
...
- transport choice와 authorization boundary를 같은 것으로 설명하면 안 된다.
