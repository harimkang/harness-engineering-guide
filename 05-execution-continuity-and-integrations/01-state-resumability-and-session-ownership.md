...
이 장의 reader-facing 외부 검증 축은 [../00-front-matter/03-references.md](../00-front-matter/03-references.md)의 Part 5 cluster를 따른다. 핵심 source ID는 `S6`, `S8`, `S26`, `S27`, `S28`이며, `P1`은 durable artifact 비교의 보조 프레임으로만 사용한다.
...
reader-facing 관점에서 이 차이를 더 빠르게 고정하려면 preservation matrix를 같이 붙이는 편이 좋다.

| artifact | 주로 보존하는 것 | 보존하지 않는 것 |
| --- | --- | --- |
| transcript | 메시지 순서, turn chain, 일부 metadata | 현재 foreground owner, live abort handle |
| checkpoint/restore state | resume에 필요한 owner-specific injection 상태 | 모든 future policy decision |
| worktree/session workspace | 파일계와 작업 디렉터리 continuity | 대화 reasoning 전체 |
| cost/attribution state | usage와 burden 누적치 | message meaning 자체 |
| config/settings | 규칙, trust, 최근 상태, preference | 현재 turn의 live control flow |
| memory store | future recall용 지식 | 현재 세션의 authoritative transcript |
...
이 지점은 LangGraph의 persistence/interrupts 문서와도 잘 맞는다. resumability 문서가 좋은지 보려면 무엇을 저장하느냐만이 아니라, 무엇은 의도적으로 저장하지 않느냐와 어떤 restore path로 다시 live state에 붙이느냐를 같이 적어야 한다.
...
- preservation matrix가 없으면 continuity 논의는 쉽게 "transcript만 있으면 된다"로 축소된다.
...
- transcript, checkpoint, worktree, config, memory가 각각 무엇을 보존하고 무엇을 보존하지 않는지 표로 적어 보라.
- startup resume와 in-session resume에서 owner가 어떻게 달라지는지 설명할 수 있어야 한다.
- restore 경로에서 의도적으로 버리는 상태가 없다면 오히려 경계가 불분명한 설계일 수 있다.
