...
이 장의 reader-facing 외부 검증 축은 [../00-front-matter/03-references.md](../00-front-matter/03-references.md)의 Part 4 cluster를 따른다. 핵심 source ID는 `S3`, `S9`, `S16`, `S17`, `S18`, `S19`, `S20`, `S23`, `S25`이며, `P2`는 benchmark optimization framing의 보조 프레임으로만 사용한다.
...
같은 이유로 operator burden과 permission fatigue도 별도 지표가 필요하다. 승인 수가 적어 보여도 민감 action이 설명 없이 자동 통과되면 좋은 tool surface라고 보기 어렵다.
...
또 하나 빠지기 쉬운 축은 observability adjacency다. trace나 run artifact가 없으면 tool surface benchmark는 성공/실패만 남고, discoverability failure인지 permission confusion인지 authorization mismatch인지 구분하기 어렵다. 이 장은 [../05-execution-continuity-and-integrations/08-observability-traces-and-run-artifacts.md](../05-execution-continuity-and-integrations/08-observability-traces-and-run-artifacts.md)와 붙여 읽는 편이 맞다.
...
- traceability와 operator burden을 빼면 tool surface benchmark는 원인 분석력이 약해진다.
