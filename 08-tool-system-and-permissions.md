...
이 장의 목적은 Claude Code의 tool system을 단순한 함수 집합이 아니라, 모델에게 노출되는 capability surface와 그 주위를 둘러싼 permission boundary를 함께 구현하는 계층으로 읽게 만드는 데 있다. 일반적으로 agent tool layer는 두 질문으로 읽는 편이 좋다. 모델이 무엇을 보게 되는가, 그리고 실제 호출 직전에는 어떤 추가 경계가 작동하는가. Claude Code의 local code는 이 두 질문에 대해 각각 `src/tools.ts`와 `src/utils/permissions/`로 다른 답을 준다.
