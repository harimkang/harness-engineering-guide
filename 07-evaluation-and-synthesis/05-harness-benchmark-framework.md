...
- long-running agent harness를 비교하기 위한 benchmark axes
...
- [../02-runtime-and-session-start/04-claude-code-architecture-map.md](../02-runtime-and-session-start/04-claude-code-architecture-map.md)
- [../03-context-and-control/05-claude-code-context-assembly-and-query-pipeline.md](../03-context-and-control/05-claude-code-context-assembly-and-query-pipeline.md)
- [../04-interfaces-and-operator-surfaces/07-claude-code-tool-system-and-permissions.md](../04-interfaces-and-operator-surfaces/07-claude-code-tool-system-and-permissions.md)
- [../05-execution-continuity-and-integrations/06-claude-code-task-model-and-background-execution.md](../05-execution-continuity-and-integrations/06-claude-code-task-model-and-background-execution.md)
- [./07-claude-code-end-to-end-scenarios.md](./07-claude-code-end-to-end-scenarios.md)
- [../03-context-and-control/](../03-context-and-control)
- [../05-execution-continuity-and-integrations/](../05-execution-continuity-and-integrations)
- [../06-boundaries-deployment-and-safety/](../06-boundaries-deployment-and-safety)
...
이 표가 의미하는 것은 특정 제품에 기능이 많다는 뜻이 아니다. long-running agent harness에서 load-bearing한 축이 무엇인지 드러난다는 뜻이다.
...
이 rubric은 단순하지만 충분히 유용하다. long-running agent harness를 비교할 때는 평균 점수보다 "어느 axis가 `부재` 또는 `국소적`인가"를 먼저 보는 편이 낫다. weakest dimension이 전체 경험을 자주 결정하기 때문이다.
...
새로운 long-running agent harness를 하나 본다고 가정해 보자.
...
1. 이 framework로 다른 long-running agent harness를 실제로 분해해 비교할 수 있는가.
...
harness benchmark framework의 핵심은 단일 점수표가 아니라 구조적 질문지다. runtime topology에서 evaluation readiness까지 이어지는 축으로 시스템을 분해해 보면, 무엇이 load-bearing이고 어디가 weakest dimension인지 더 빨리 드러난다. 그때 비로소 benchmark는 비교가 아니라 설계 도구가 된다.
