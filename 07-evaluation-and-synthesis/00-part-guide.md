...
이 Part는 harness를 측정 가능한 시스템으로 읽는 방법과, 앞선 모든 축을 다시 종합하는 방법을 다룹니다. 먼저 model eval과 harness eval의 차이, task/trial/transcript/grader vocabulary, benchmark frame, production trace loop, skeptical evaluator를 일반론으로 읽고, 이어서 Claude Code end-to-end scenario와 code reading guide를 통해 전체 구조를 다시 묶습니다.
